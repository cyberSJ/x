...
void initMenu()
{
    // First Entry - top level menu
    h_first_entry = menu_handler.insert( "First Entry" );

    // First Entry
    // |- deep
    MenuHandler::EntryHandle entry = menu_handler.insert( h_first_entry, 
                                                          "deep" );
    ...
    //    |- sub
    //       |- menu --> deepCb();
    // Show First Entry --> enableCb(); 
    menu_handler.setCheckState( 
        menu_handler.insert( "Show First Entry", &enableCb ),
        MenuHandler::CHECKED );

    MenuHandler::EntryHandle sub_menu_handle = menu_handler.insert( "Switch" );

    // Each of the sub-menu has a check box on the left. Unckecked initially.
    for( int i = 0; i < 5; ++i )
    {
        std::ostringstream s;
        s << "Mode " << i;
    ...
    // sung why check the very last entry? It's like programming a default
    // behavior.
    menu_handler.setCheckState( h_mode_last, MenuHandler::CHECKED );
}
